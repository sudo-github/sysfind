...
if [ ! -z "${TOOLS_PREFIX}" ]; then
	...
	for fname in `echo -n /usr/local/bin/????version | sed 's/^\(.*\)\(\/usr\/local\/bin\/vsmpversion\)\(.*\)$/\1\3 \2/'`; do
		...
		if [ $? == 0 ]; then TOOL=`basename $fname | awk '{print substr($0,1,4)}'`; break; fi
	done
fi

# Setting NNA has been disabled due to its disruptive nature
#${TOOL}ctl --nna=on
...
for x in $((VSMP_CPUS_PER_BOARD*VSMP_BOARD_COUNT/threads)) ; do
    ...
    for factor in $FAC ; do
      for size in $(( factor * seg )) ; do
        ...
        for cycle in 1 ; do
          ...
          numactl --interleave all /usr/bin/time -p $CMD $size $size $seg $seg $seg $seg 2>&1 | tee -a log-gemm-$factor-$x-$size-$seg-seg-ilv-$cycle.$TAG.txt | tee -a /dev/tty | tee -a /dev/tty
        done
      done
    done
  done
